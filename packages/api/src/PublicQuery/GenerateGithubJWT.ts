...
export const handler = async (input: FieldResolveInput) =>
  resolverFor('PublicQuery', 'GenerateGithubJWT', async (args) => {
    ...
    if (!githubProfile) throw new Error('cannot fetch github profile');
    const githubEmails = await getUserInfo('https://api.github.com/user/emails', github_access_token);
    if (!githubEmails.length) throw new Error('email length empty');
